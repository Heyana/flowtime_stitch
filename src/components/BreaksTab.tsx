...
export default function BreaksTab({ onStartBreathe }: BreaksTabProps) {
  // Hydration state persisted to localStorage
  ...
  useEffect(() => {
    localStorage.setItem('flowtime_water_count', waterCount.toString());
  }, [waterCount]);
  ...
  useEffect(() => {
    ...
    if (stretchActive && stretchTimeLeft > 0) {
      ...
      if (stretchIndex < stretches.length - 1) {
        ...
        alert('Stretching complete! Feel refreshed?');
        setStretchIndex(0);
        setStretchTimeLeft(30);
      }
    }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col gap-8 pb-12 animate-fade-in">
      ...
      <section className="flex flex-col gap-1.5 text-center md:text-left">
        <h2 className="font-display text-3xl md:text-4xl font-bold text-on-surface">Recharge Center</h2>
        <p className="font-sans text-sm md:text-base text-on-surface-variant">
          Take care of your mind and body during transitions and breaks.
        </p>
      </section>

      {/* Grid of Breaks & Trackers */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        
        {/* Mindfulness Exercises Catalog */}
        <div className="flex flex-col gap-4">
          <h3 className="text-xs font-semibold text-on-surface-variant tracking-wider uppercase pl-1">
            Recharge Activities
          </h3>
          <div className="flex flex-col gap-3">
            ...
            <div className="bg-surface-container-lowest shadow-soft rounded-xl p-5 flex items-center gap-4 border border-transparent hover:border-surface-container-high transition-all">
              ...
              <div className="flex-grow">
                <h4 className="text-sm font-semibold text-on-surface">Outdoor/Short Walk</h4>
                <p className="text-xs text-on-surface-variant mt-0.5">Stretch your legs and get offline for 5 mins.</p>
              </div>
            </div>

            {/* Deep Breathing Activator */}
            <div className="bg-surface-container-lowest shadow-soft rounded-xl p-5 flex items-center gap-4 border border-transparent hover:border-surface-container-high transition-all">
              <Wind className="w-5 h-5 text-secondary flex-shrink-0" />
              <div className="flex-grow">
                <h4 className="text-sm font-semibold text-on-surface">Guided Breathing</h4>
                <p className="text-xs text-on-surface-variant mt-0.5">Regulate your nervous system with visuals.</p>
              </div>
              <button
        ...
        <div className="bg-surface-container-lowest rounded-xl shadow-soft p-6 flex flex-col gap-5 border border-transparent hover:border-surface-container-high transition-all">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-semibold text-on-surface-variant tracking-wider uppercase">Liquid Hydration</h3>
          </div>

          {/* Visual Cup indicator (Liquid animation) */}
          <div className="flex flex-col items-center gap-3 py-4 bg-surface-container-low/50 rounded-xl relative overflow-hidden">
            {/* Wavy background for liquid effect */}
            <div className="text-center z-10 flex flex-col gap-1">
              <span className="font-display text-4xl font-bold text-secondary tabular-nums">
                {waterCount} <span className="text-base font-normal text-on-surface-variant">/ 8 cups</span>
              </span>
              <span className="text-xs font-medium text-on-surface-variant">
                {waterCount >= 8 ? 'Hydrated & Energized!' : 'Keep sipping to prevent brain fatigue'}
              </span>
            </div>

            {/* Visual Glass filling based on water count */}
              ...
              {Array.from({ length: 8 }).map((_, idx) => (
                <div 
                  ...
                  className={`w-4 h-8 rounded-full border border-secondary/30 transition-all duration-300 ${
                    idx < waterCount 
                      ? 'bg-secondary/60 scale-y-105 shadow-[0_2px_8px_rgba(0,109,62,0.2)]' 
                      : 'bg-transparent'
                  }`}
                />
              ))}
        ...
        {/* Guided Stretch Timer (Bento Wide) */}
        <div className="bg-surface-container-lowest rounded-xl shadow-soft p-6 flex flex-col gap-4 border border-transparent hover:border-surface-container-high transition-all md:col-span-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-semibold text-on-surface-variant tracking-wider uppercase">Desk Wellness</h3>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-center bg-surface-container-low/30 p-5 rounded-xl">
            {/* Stretches list index */}
            ...
            <div className="flex flex-col items-center text-center px-4 py-2 border-y sm:border-y-0 sm:border-x border-surface-container">
              <span className="font-display text-4xl font-extrabold text-primary tabular-nums">
                {stretchTimeLeft}s
              </span>
              <span className="text-xs font-medium text-on-surface-variant mt-1.5">
                {stretchActive ? 'Stretch & breathe deeply...' : 'Ready to stretch?'}
              </span>
            ...
            <div className="flex flex-col gap-3">
              <div className="bg-surface-container-lowest p-3 rounded-lg border border-surface-container-high/50 shadow-xs">
                <h5 className="text-xs font-bold text-primary tracking-wider uppercase">Current Move</h5>
              ...
              <div className="flex gap-2">
                ...
                >
                  {stretchActive ? 'Pause' : 'Start Stretch'}
                </button>
                <button
...
// Inline helper for refresh icon since it is highly useful
