...
export default function HistoryTab({ sessions, streak, onAddSession, onClearHistory, onDeleteSession }: HistoryTabProps) {
  ...
  const totalFocusMinutes = todaySessions.filter(s => s.type === 'work').reduce((acc, curr) => acc + curr.durationMinutes, 0);
  const totalRestMinutes = todaySessions.filter(s => s.type === 'rest').reduce((acc, curr) => acc + curr.durationMinutes, 0);
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    onAddSession({ type: newType, title: newTitle, durationMinutes: Number(newDuration), startTime: newStart, endTime: newEnd, date: todayStr });
    setNewTitle('');
    setNewDuration(25);
    ...
    <div className="w-full max-w-4xl mx-auto flex flex-col gap-8 pb-12 animate-fade-in">
      <section className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 text-center md:text-left">
        <div className="flex flex-col gap-1.5">
          <h2 className="font-display text-3xl md:text-4xl font-bold text-on-surface">{t('history.title')}</h2>
          <p className="font-sans text-sm md:text-base text-on-surface-variant">
            {t('history.totalFocus')}: <span className="font-semibold text-primary">{formatMinutes(totalFocusMinutes)}</span>
          </p>
        </div>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-surface-container-lowest rounded-xl p-5 shadow-soft flex flex-col justify-between relative overflow-hidden min-h-[104px]">
          <div className="flex items-center gap-2"><Flame className="w-4 h-4 fill-current text-primary" /><span className="text-xs text-on-surface-variant font-medium tracking-wider uppercase">{t('history.streak')}</span></div>
          <p className="font-display text-2xl font-bold text-on-surface">{streak} {t('unit.days')}</p>
        </div>
        <div className="bg-surface-container-lowest rounded-xl p-5 shadow-soft flex flex-col justify-between relative overflow-hidden min-h-[104px]">
      ...
      <section className="flex flex-col gap-4">
        <h3 className="text-xs font-semibold text-on-surface-variant tracking-wider uppercase pl-1 border-b border-surface-container-high pb-2">{t('history.timeline')}</h3>
        {todaySessions.length === 0 ? (
          <div className="text-center py-12 bg-surface-container-lowest rounded-xl shadow-soft flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-medium text-on-surface-variant">{t('history.empty')}</p>
            <p className="text-xs text-on-surface-variant/60">{t('history.emptyHint')}</p>
          </div>
        ) : (
            ...
            {todaySessions.map((session) => {
              ...
              return (
                <div key={session.id} className="relative flex items-start gap-4 group">
                  <div className={`absolute -left-6 sm:-left-8 w-4.5 h-4.5 rounded-full border-4 border-background mt-1 transition-transform group-hover:scale-110 ${isWork ? 'bg-primary' : 'bg-secondary'}`} />
                  <div className="bg-surface-container-lowest rounded-xl p-4 sm:p-5 shadow-soft flex-grow flex justify-between items-center transition-all duration-200 hover:bg-surface-container-low border border-transparent hover:border-surface-container-high">
                    <div className="flex flex-col gap-1 pr-4">
                      <span className="text-sm sm:text-base font-semibold text-on-surface leading-snug">{session.title}</span>
                    ...
                    <div className="flex items-center gap-3">
                      {isWork ? (
                        <div className="bg-primary/10 text-primary px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 tracking-wider"><Clock className="w-3.5 h-3.5" />{t('history.focus')}</div>
                      ) : (
                        <div className="bg-secondary/10 text-secondary px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 tracking-wider"><Coffee className="w-3.5 h-3.5" />{t('history.rest')}</div>
                      )}
                      {deleteConfirmId === session.id ? (
                        <div className="flex items-center gap-1">
                          <span className="text-[10px] text-error font-semibold">{t('history.deleteConfirm')}</span>
                          <button onClick={() => { onDeleteSession(session.id); setDeleteConfirmId(null); }} className="p-1 text-error hover:bg-error/10 rounded-full transition-colors cursor-pointer"><Check className="w-3.5 h-3.5" /></button>
                          <button onClick={() => setDeleteConfirmId(null)} className="p-1 text-on-surface-variant/60 hover:text-on-surface rounded-full hover:bg-surface-container transition-colors cursor-pointer"><X className="w-3.5 h-3.5" /></button>
                        </div>
                      ) : (
                        <button onClick={() => setDeleteConfirmId(session.id)} className="p-1.5 text-on-surface-variant/40 hover:text-error rounded-full hover:bg-error/5 transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100 cursor-pointer"><X className="w-4 h-4" /></button>
                      )}
                    </div>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-on-surface/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-surface-container-lowest w-full max-w-md rounded-xl shadow-soft p-6 flex flex-col gap-6 animate-scale-up">
            <div className="flex justify-between items-center border-b border-surface-container pb-3">
              <h3 className="font-display text-lg font-bold text-on-surface">{t('history.logModal.title')}</h3>
              <button onClick={() => setShowAddModal(false)} className="p-1 text-on-surface-variant/60 hover:text-on-surface rounded-full hover:bg-surface-container transition-colors cursor-pointer"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={handleAddSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-on-surface-variant font-medium tracking-wider uppercase">{t('history.logModal.titleLabel')}</label>
                <input type="text" required placeholder={newType === 'work' ? t('history.logModal.focusPlaceholder') : t('history.logModal.restPlaceholder')}
              ...
              <div className="flex flex-col gap-1.5">
                <div className="flex justify-between items-center text-xs text-on-surface-variant font-medium">
                  <span className="tracking-wider uppercase">{t('history.logModal.duration')}</span>
                  <span className="text-on-surface font-semibold">{newDuration} {t('unit.minutes')}</span>
                </div>
                <input type="range" min="5" max="120" step="5" value={newDuration} onChange={(e) => setNewDuration(Number(e.target.value))} className="w-full accent-primary" />
              </div>
